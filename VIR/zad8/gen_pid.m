...
tout1 = [0;10;30;50;70;90]';
w1 = [5;4.5;6;3;7;5.5]';
tw1 = [tout1;w1]';

numgen=30;	% number of generations
lpop=56*4;	% number of chromosomes in population
...
for gen=1:numgen
    ...
    [Best,bfit]=selbest(Pop,Fit,[1*4,1*4]);          % optimum
    best_ch(:,gen) = Best(1,:)';
    Old=selrand(Pop,Fit,8*4);                     % vyssii selektivny tlak avsak so
    Work1=seltourn(Pop,Fit,8*4);               % zachovanim nahodneho vyberu
    Work1=[Work1; selrand(Pop,Fit,8*4)];      % pomerne vysoka mutacia zvisuje diverzitu
    Work2=seltourn(Pop,Fit,8*4);
    Work2=[Work2; selsus(Pop,Fit,8*4)];
    Work4=seltourn(Pop,Fit,8*4);
